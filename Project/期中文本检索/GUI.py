...
class MainPanel:
    ...
    def check_new_searchterm(self):
        ...
        if len(terms) == 0 or len(terms) > 3:
            ...
            self.search_request(searchterm)
    ...
    def search_request(self, terms):
        ...
        client.send(terms.encode('utf-8'))
        result = client.recv(65536).decode('utf-8', errors='ignore')
        result = result.split('~')
        result = [i for i in result if i != '']
        self.documents = []
        # 只记录title和body
        for i in range(len(result)//2):
            self.documents.append((result[2*i], result[2*i+1]))
